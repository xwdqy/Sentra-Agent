...
const PRESET_CONVERTER_PROMPT_NAME = 'sentra_preset_converter_prompt_system';
let cachedPresetConverterSystemPrompt: string | null = null;
...
type AgentLike = {
  chat: (messages: ChatMessage[], options?: Record<string, unknown>) => Promise<unknown>;
};

type ConvertPresetOptions = {
  agent?: AgentLike;
  rawText?: string;
  fileName?: string;
  model?: string;
...
function getPresetCachePath(fileName: string | undefined, signatureText: string | undefined): { cacheDir: string; cacheFile: string } {
  ...
  const suffix = sigHash ? `.${takePrefix(sigHash, 12)}` : '';
  const cacheFile = path.join(cacheDir, `${safeName}${suffix}.json`);
  return { cacheDir, cacheFile };
}

function tryLoadPresetCache(rawText: string | undefined, fileName?: string, signatureText?: string): PresetJson | null {
  ...
  try {
    ...
    const data = JSON.parse(content) as { textHash?: string; presetJson?: PresetJson };
...
function savePresetCache(presetJson: PresetJson, rawText: string | undefined, fileName?: string, signatureText?: string): void {
  const text = typeof rawText === 'string' ? rawText : '';
...
function extractTagText(xml: string | null | undefined, tagName: string): string {
...
function parseSimpleChildrenBlock(inner: string | null | undefined): Record<string, string | string[]> {
...
function parseSentraAgentPresetXml(xml: string): PresetJson | null {
  if (!xml) return null;
  const metaXml = extractFirstTagBlock(xml, 'meta');
  const meta: PresetMeta = {};
  const metaKeys = ['node_name', 'category', 'description', 'version', 'author'] as const;
  ...
  if (rulesInner) {
    ...
    while ((m = reRule.exec(src)) !== null) {
      ...
      const conditions: PresetRuleCondition[] = [];
      if (conditionsInner) {
 ...
 *
 * @param {Object} params
 * @param {import('../src/agentRuntime.js').Agent} params.agent - 已初始化的 Agent 实例
 ...
 */
export async function convertPresetTextToJson({ agent, rawText, fileName, model }: ConvertPresetOptions = {}) {
