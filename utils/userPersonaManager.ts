...
let cachedPersonaInitialSystemPrompt: string | null = null;
let cachedPersonaRefineSystemPrompt: string | null = null;
...
export interface PersonaAgent {
  chat: (conversations: Array<{ role: string; content: string }>, options?: Record<string, unknown>) => Promise<unknown>;
}

export interface UserPersonaManagerOptions {
  agent: PersonaAgent;
  dataDir?: string;
  enabled?: boolean;
  updateIntervalMs?: number;
  minMessagesForUpdate?: number;
...
interface PersonaStats {
  traits: Record<string, PersonaStatEntry>;
  interests: Record<string, PersonaStatEntry>;
  patterns: Record<string, PersonaStatEntry>;
  insights: Record<string, PersonaStatEntry>;
}

interface UserData {
  senderId: SenderId;
  // persona 是 LLM 生成的动态结构，历史上按 JS 方式自由访问字段；严格 TS 下保持 any 以避免破坏既有行为
  persona: any;
  messages: unknown[];
  messageCount: number;
...
class UserPersonaManager {
  ...
  cache: Map<SenderId, UserData>;
  pendingUpdates: Set<SenderId>;

  constructor(options: UserPersonaManagerOptions) {
    // 配置参数
    ...
    this._applyConfig(this._getRuntimeDefaults(), options as unknown as Record<string, unknown>);
  ...
  _getRuntimeDefaults(): Record<string, unknown> {
    return {
  ...
  _applyConfig(defaults: Record<string, unknown>, overrides: Record<string, unknown>) {
   ...
   * 获取用户数据文件路径
   */
  _getUserFilePath(senderId: SenderId): string {
    // 安全的文件名（移除特殊字符）
   ...
   * 加载用户数据
   */
  _loadUserData(senderId: SenderId): UserData | null {
    // 先从缓存读取
   ...
   * 保存用户数据
   */
  _saveUserData(senderId: SenderId, data: UserData): boolean {
    const filePath = this._getUserFilePath(senderId);
   ...
   */
  async recordMessage(senderId: SenderId, message: Record<string, any>): Promise<void> {
   ...
   * @private
   */
  _checkAndScheduleUpdate(senderId: SenderId, userData: UserData): void {
    // 1. 检查是否已有待执行的更新（防重）
   ...
   * 更新用户画像（使用 LLM 分析）
   */
  async updatePersona(senderId: SenderId): Promise<void> {
    const userData = this._loadUserData(senderId);
  ...
  async _analyzePersona(
    ...
    try {
      ...
      const responseText = responseAny?.content || (typeof response === 'string' ? response : '');
   ...
   */
  async _getSystemPrompt(isFirstTime: boolean): Promise<string> {
    ...
    try {
      ...
      const system = await SentraPromptsSDK(
        isFirstTime
          ? "{{sentra_persona_initial_prompt_system}}"
          : "{{sentra_persona_refine_prompt_system}}",
        PROMPTS_CONFIG_PATH
      );

      if (system) {
        if (isFirstTime) {
          cachedPersonaInitialSystemPrompt = String(system);
        } else {
          cachedPersonaRefineSystemPrompt = String(system);
        }
   ...
   */
  _buildAnalysisPrompt(recentMessages: any[], existingPersona: any, isFirstTime: boolean, senderId: SenderId): string {
    let prompt = '';
   ...
   */
  _parsePersonaXML(personaXML: string): any {
    const persona: any = {};
  ...
  _serializePersonaToXML(persona: any, senderId: SenderId): string {
    ...
    const escText = (v: unknown) => escapeXml(unescapeXml(s(v)));
    const escAttr = (v: unknown) => escapeXmlAttr(unescapeXml(s(v)));
    const arr = (a: any): any[] => Array.isArray(a) ? a : (a ? [a] : []);
    const items = (a: any): PersonaListItem[] => arr(a)
      .map((x: any): PersonaListItem => (typeof x === 'object' && x && (x.content || x.attributes))
        ? { content: s(x.content), attributes: (x.attributes && typeof x.attributes === 'object') ? x.attributes : {} }
        : { content: s(x), attributes: {} })
      .filter((i: PersonaListItem) => i.content);
    const attrs = (o: any, ks: string[]): string => {
      const ps: string[] = [];
      ks.forEach((k) => { if (o && o[k] != null && String(o[k]).trim()) ps.push(`${k}="${escAttr(o[k])}"`); });
   ...
   * 获取画像摘要（用于日志输出）
   */
  _getPersonaSummary(persona: any): string {
  ...
  _mergePersonaWithStats(userData: UserData, newPersona: any): unknown {
    const now = Date.now();
    if (!userData.personaStats) {
      userData.personaStats = { traits: {}, interests: {}, patterns: {}, insights: {} };
    }
    const stats: PersonaStats = userData.personaStats;
    const readItems = (arr: unknown): PersonaListItem[] => {
      if (!arr) return [];
      if (!Array.isArray(arr)) return [];
      return (arr as unknown[])
        .map((x): PersonaListItem => {
          if (typeof x === 'object' && x && ('content' in (x as any) || 'attributes' in (x as any))) {
            const anyX = x as any;
            return { content: String(anyX.content || '').trim(), attributes: (anyX.attributes && typeof anyX.attributes === 'object') ? anyX.attributes : {} };
          }
          return { content: String(x || '').trim(), attributes: {} };
        })
        .filter((it) => it.content);
    };
    const decay = (w: number, lastSeen: number | undefined) => {
    ...
    const bump = (base: number, attrs: Record<string, unknown> | undefined) => {
      let b = base;
      const s = (attrs as any)?.status;
      if (s === 'confirmed') b += 0.2;
      else if (s === 'refined') b += 0.1;
      else if (s === 'new') b += 0.15;
      const n = (attrs as any)?.novelty;
      if (n === 'new') b += 0.15;
      const t = (attrs as any)?.trend;
      if (t === '增强') b += 0.1;
      return b;
    };
    const normKey = (s: unknown) => String(s || '').toLowerCase().replace(/\s+/g, ' ').trim().slice(0, 200);
    const decayAll = (map: Record<string, PersonaStatEntry>) => {
      Object.keys(map).forEach((k) => {
        const entry = map[k];
        if (!entry) return;
        entry.weight = decay(entry.weight || 0, entry.lastSeen);
      });
    };
    const upsert = (map: Record<string, PersonaStatEntry>, item: PersonaListItem, base: number) => {
    ...
    const topN = (map: Record<string, PersonaStatEntry>, n: number) => {
   ...
   * 获取用户画像（供 AI 使用）
   */
  getPersona(senderId: SenderId): any {
    const userData = this._loadUserData(senderId);
   ...
   */
  formatPersonaForContext(senderId: SenderId): string {
    if (!this.enabled) return '';
   ...
   * 强制更新画像（手动触发）
   */
  async forceUpdate(senderId: SenderId): Promise<void> {
   ...
   * 重置用户画像
   */
  resetPersona(senderId: SenderId): void {
    const userData = this._loadUserData(senderId);
    if (userData) {
   ...
   * 获取用户统计信息
   */
  getUserStats(senderId: SenderId): Record<string, unknown> | null {
    const userData = this._loadUserData(senderId);
   ...
   * 列出所有用户
   */
  listAllUsers(): Array<Record<string, unknown>> {
