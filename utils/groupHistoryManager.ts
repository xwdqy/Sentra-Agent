...
type PairId = string | number;

type PairRole = 'user' | 'assistant';
...
type ActivePairStatus = 'building' | 'finished' | 'cancelled';
type ActivePairCommitMode = 'shared' | 'scoped';

type ActivePairMessage = {
  role: PairRole;
  content: string;
};

type ActivePairContext = {
  assistant: string;
  userContent: string | null;
  messages: ActivePairMessage[];
  createdAt: number;
  lastUpdatedAt: number;
  status: ActivePairStatus;
  scopeSenderId: string | null;
  commitMode: ActivePairCommitMode;
};

type GroupHistory = {
  conversations: ConversationMessage[];
  scopedConversationsBySender: Map<string, ConversationMessage[]>;
  pendingMessages: PendingMessage[];
  processingMessages: PendingMessage[];
  activePairs: Map<string, ActivePairContext>;
  senderLastMessageTime: Map<string, number>;
};

type SerializedGroupHistory = {
  conversations: ConversationMessage[];
  scopedConversationsBySender: Record<string, ConversationMessage[]>;
  pendingMessages: PendingMessage[];
  processingMessages: PendingMessage[];
  activePairs: Record<string, ActivePairContext>;
  senderLastMessageTime: Record<string, number>;
};

type GroupQueueTask<T> = () => Promise<T>;
type GroupQueueItem<T> = {
  task: GroupQueueTask<T>;
  resolve: (value: T) => void;
  reject: (error: unknown) => void;
};

type DecisionRecentOptions = {
  groupLimit?: number;
  senderLimit?: number;
  botLimit?: number;
  maxChars?: number;
...
type DecisionRecentResult = {
  group_recent_messages: DecisionRecentMessage[];
  sender_recent_messages: DecisionRecentMessage[];
  bot_recent_messages: DecisionBotMessage[];
};

type PendingMessagesAIFormat = {
  xml: string;
  objective: string;
  hasContext: boolean;
  targetMsg: RawMessage | null;
};

type StartAssistantMessageOptions = {
  scopeSenderId?: SenderId;
  commitMode?: ActivePairCommitMode;
};
...
function getTokenCounterSingleton(): TokenCounter {
...
function getDecisionGroupRecentMessagesDefault(): number | undefined {
  return getEnvInt('REPLY_DECISION_GROUP_RECENT_MESSAGES', 15);
}

function getDecisionSenderRecentMessagesDefault(): number | undefined {
  return getEnvInt('REPLY_DECISION_SENDER_RECENT_MESSAGES', 5);
}

function getDecisionBotRecentMessagesDefault(): number | undefined {
...
class GroupTaskQueue extends EventEmitter {
  running: number;
  queue: Array<GroupQueueItem<unknown>>;
   ...
   */
  async pushTask<T>(task: GroupQueueTask<T>): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      const item: GroupQueueItem<unknown> = {
        task: task as GroupQueueTask<unknown>,
 ...
 */
function serializeHistory(history: GroupHistory): SerializedGroupHistory {
  const senderLastMessageTimeObj: Record<string, number> = {};
  if (history.senderLastMessageTime && history.senderLastMessageTime instanceof Map) {
    for (const [senderId, ts] of history.senderLastMessageTime.entries()) {
      const timeValue = typeof ts === 'number' ? ts : Number(ts);
      senderLastMessageTimeObj[String(senderId)] = Number.isFinite(timeValue) ? timeValue : 0;
    }
  }

  const scopedConversationsBySenderObj: Record<string, ConversationMessage[]> = {};
  ...
  const activePairsObj: Record<string, ActivePairContext> = {};
  if (history.activePairs && history.activePairs instanceof Map) {
    for (const [pairId, ctx] of history.activePairs.entries()) {
      ...
      const lastUpdatedAt = typeof ctx.lastUpdatedAt === 'number'
        ? ctx.lastUpdatedAt
        : createdAt;
      const statusRaw = typeof ctx.status === 'string' ? ctx.status : 'building';
      const status: ActivePairStatus = statusRaw === 'finished' || statusRaw === 'cancelled' ? statusRaw : 'building';
      const scopeSenderId = typeof ctx.scopeSenderId === 'string' ? ctx.scopeSenderId : null;
      const commitModeRaw = typeof ctx.commitMode === 'string' ? ctx.commitMode : 'shared';
      const commitMode: ActivePairCommitMode = commitModeRaw === 'scoped' ? 'scoped' : 'shared';
...
function normalizeConversationMessage(raw: unknown): ConversationMessage | null {
  ...
  const pairId = obj.pairId != null ? obj.pairId as PairId : undefined;
...
function normalizePendingMessage(raw: unknown): PendingMessage | null {
  if (!raw || typeof raw !== 'object') return null;
  const obj = raw as Record<string, unknown>;
  const summary = typeof obj.summary === 'string' ? obj.summary : '';
  const timestamp = typeof obj.timestamp === 'number' ? obj.timestamp : 0;
  const msgObjRaw = obj.msgObj && typeof obj.msgObj === 'object'
    ? obj.msgObj as RawMessage
    : {};
  return { summary, msgObj: msgObjRaw, timestamp };
}
 ...
 */
function deserializeHistory(data: unknown): GroupHistory | null {
  ...
  const senderLastMessageTime = new Map<string, number>();
  if (dataObj.senderLastMessageTime && typeof dataObj.senderLastMessageTime === 'object') {
    for (const [senderId, ts] of Object.entries(dataObj.senderLastMessageTime as Record<string, unknown>)) {
      const timeValue = typeof ts === 'number' ? ts : Number(ts);
      senderLastMessageTime.set(String(senderId), Number.isFinite(timeValue) ? timeValue : 0);
    }
  }

  const activePairs = new Map<string, ActivePairContext>();
  if (dataObj.activePairs && typeof dataObj.activePairs === 'object') {
    for (const [pairId, ctx] of Object.entries(dataObj.activePairs as Record<string, unknown>)) {
      if (!ctx || typeof ctx !== 'object') continue;

      const ctxObj = ctx as Record<string, unknown>;
      const createdAt = typeof ctxObj.createdAt === 'number' ? ctxObj.createdAt : 0;
      const lastUpdatedAt = typeof ctxObj.lastUpdatedAt === 'number'
        ? ctxObj.lastUpdatedAt
        : createdAt;
      const assistant = typeof ctxObj.assistant === 'string' ? ctxObj.assistant : '';
      const userContent = typeof ctxObj.userContent === 'string' ? ctxObj.userContent : null;
      const messages = Array.isArray(ctxObj.messages) ? ctxObj.messages : [];
      const statusRaw = typeof ctxObj.status === 'string' ? ctxObj.status : 'building';
      const status: ActivePairStatus = statusRaw === 'finished' || statusRaw === 'cancelled' ? statusRaw : 'building';
      const commitModeRaw = typeof ctxObj.commitMode === 'string' ? ctxObj.commitMode : 'shared';
      const commitMode: ActivePairCommitMode = commitModeRaw === 'scoped' ? 'scoped' : 'shared';
      ...
      activePairs.set(String(pairId), {
        ...
        userContent,
        messages: messages
          .map((m) => normalizeConversationMessage(m))
          .filter((m): m is ConversationMessage => !!m)
          .map((m) => ({ role: m.role, content: m.content })),
        createdAt,
        lastUpdatedAt,
        status,
        scopeSenderId: typeof ctxObj.scopeSenderId === 'string' ? ctxObj.scopeSenderId : null,
        commitMode
      });
    }
  }

  const scopedConversationsBySender = new Map<string, ConversationMessage[]>();
  if (dataObj.scopedConversationsBySender && typeof dataObj.scopedConversationsBySender === 'object') {
    for (const [senderId, arr] of Object.entries(dataObj.scopedConversationsBySender as Record<string, unknown>)) {
      if (!senderId) continue;
      if (!Array.isArray(arr) || arr.length === 0) continue;
      ...
      if (normalized.length > 0) {
        scopedConversationsBySender.set(String(senderId), normalized);
      }
    }
  }

  return {
    conversations: Array.isArray(dataObj.conversations)
      ? dataObj.conversations
        .map((m) => normalizeConversationMessage(m))
        .filter((m): m is ConversationMessage => !!m)
      : [],
    scopedConversationsBySender,
    pendingMessages: Array.isArray(dataObj.pendingMessages)
      ? dataObj.pendingMessages
        .map((pm) => normalizePendingMessage(pm))
        .filter((pm): pm is PendingMessage => !!pm)
      : [],
    processingMessages: Array.isArray(dataObj.processingMessages)
      ? dataObj.processingMessages
        .map((pm) => normalizePendingMessage(pm))
        .filter((pm): pm is PendingMessage => !!pm)
      : [],
    activePairs,
...
async function loadHistoryFromRedis(groupId: GroupId): Promise<GroupHistory | null> {
...
async function saveHistoryToRedis(groupId: GroupId, history: GroupHistory): Promise<void> {
  const redis = getRedis();
  if (!redis) return;
...
async function deleteHistoryFromRedis(groupId: GroupId): Promise<void> {
  const redis = getRedis();
  if (!redis) return;
 ...
 */
type GroupHistoryManagerOptions = {
  maxConversationPairs?: number | undefined;
};

class GroupHistoryManager {
  maxConversationPairs: number;
  histories: Map<GroupId, GroupHistory>;
  queues: Map<GroupId, GroupTaskQueue>;

  constructor(options: GroupHistoryManagerOptions = {}) {
    // 配置
   ...
   * @returns {Promise} 任务执行结果
   */
  async _executeForGroup<T>(groupId: GroupId, task: () => Promise<T>): Promise<T> {
    // 确保该群有队列
    ...
    if (!queue) {
      throw new Error('group_queue_missing');
    }
   ...
   */
  async _getOrInitHistory(groupId: GroupId): Promise<GroupHistory> {
    const existing = this.histories.get(groupId);
    if (existing) {
      return existing;
    }
   ...
   */
  async addPendingMessage(groupId: GroupId, summary: string, msgObj: RawMessage): Promise<void> {
  ...
  async appendToConversationPairMessages(groupId: GroupId, pairId: PairId, role: string, content: unknown): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.activePairs || !(history.activePairs instanceof Map)) {
  ...
  async promoteScopedConversationsToShared(groupId: GroupId, senderId: SenderId): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.scopedConversationsBySender || !(history.scopedConversationsBySender instanceof Map)) {
      ...
      try {
        ...
        if (Number.isFinite(limit) && limit > 0) {
          ...
          if (orderedPairIds.length > limit) {
            const drop = new Set<string>(orderedPairIds.slice(0, orderedPairIds.length - limit));
  ...
  async clearScopedConversationsForSender(groupId: GroupId, senderId: SenderId): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.scopedConversationsBySender || !(history.scopedConversationsBySender instanceof Map)) {
        history.scopedConversationsBySender = new Map();
   ...
   * @private
   */
  _cleanupTimeoutMessages(history: GroupHistory, now: number, timeoutMs: number = 2 * 60 * 1000): { cleaned: number; details: string[] } {
    const timeoutSenders: string[] = [];
    const cleaned: { cleaned: number; details: string[] } = { cleaned: 0, details: [] };
   ...
   */
  getPendingMessagesXML(groupId: GroupId): string {
   ...
   */
  getPendingMessagesContent(groupId: GroupId): string {
   ...
   */
  getPendingMessagesContext(groupId: GroupId, senderId: SenderId | null = null): string {
    const history = this.histories.get(groupId);
    if (!history) {
      return '';
    }
  ...
  formatPendingMessagesForAI(groupId: GroupId, targetSenderId: SenderId | null = null): PendingMessagesAIFormat {
    ...
    const lastMsgObj = lastMsg ? lastMsg.msgObj : null;
    const contextXml = this.getPendingMessagesContext(groupId);
    const objective = lastMsgObj
      ? (typeof lastMsgObj.text === 'string' && lastMsgObj.text.trim()
        ? lastMsgObj.text
        : (typeof lastMsgObj.summary === 'string' && lastMsgObj.summary.trim()
          ? lastMsgObj.summary
          : '瀹屾垚鐢ㄦ埛璇锋眰'))
      : '瀹屾垚鐢ㄦ埛璇锋眰';
   ...
   */
  getLastPendingMessage(groupId: GroupId): RawMessage | null {
    const history = this.histories.get(groupId);
    if (!history || history.pendingMessages.length === 0) {
      return null;
    }
    const last = history.pendingMessages[history.pendingMessages.length - 1];
    if (!last) return null;
    return last.msgObj;
  }
   ...
   * @returns {Object|null} 消息对象或null
   */
  findLastMessageBySender(groupId: GroupId, senderId: SenderId): RawMessage | null {
    const history = this.histories.get(groupId);
    if (!history) {
      return null;
    }
   ...
   */
  async startProcessingMessages(groupId: GroupId, senderId: SenderId): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
   ...
   */
  getPendingMessagesBySender(groupId: GroupId, senderId: SenderId): RawMessage[] {
    const history = this.histories.get(groupId);
  ...
  getRecentMessagesForDecision(groupId: GroupId, senderId: SenderId | null, options: DecisionRecentOptions = {}): DecisionRecentResult {
    const history = this.histories.get(groupId);
    if (!history) {
      return {
    ...
    const defaultGroupLimit = getDecisionGroupRecentMessagesDefault() ?? 0;
    const defaultSenderLimit = getDecisionSenderRecentMessagesDefault() ?? 0;
    const defaultBotLimit = getDecisionBotRecentMessagesDefault() ?? 0;
    const defaultMaxChars = getDecisionContextMaxCharsDefault() ?? 0;
    ...
    const result: DecisionRecentResult = {
      group_recent_messages: [],
      sender_recent_messages: [],
      bot_recent_messages: []
    };
    ...
    if (groupLimit > 0) {
      const groupSlice = sorted.slice(-groupLimit);
        result.group_recent_messages = groupSlice.map((pm) => {
          const msg = pm.msgObj || {};
          const rawText = typeof msg.text === 'string' ? msg.text : (pm.summary || '');
          const senderName = typeof msg.sender_name === 'string' ? msg.sender_name : 'Unknown';
          const timeStr = typeof msg.time_str === 'string' ? msg.time_str : '';
          return {
    ...
    if (botLimit > 0) {
      const botCandidates: DecisionBotMessage[] = [];
   ...
   */
  async startAssistantMessage(groupId: GroupId, options: StartAssistantMessageOptions | null = null): Promise<string> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.activePairs || !(history.activePairs instanceof Map)) {
        history.activePairs = new Map();
      ...
      history.activePairs.set(pairKey, {
        assistant: '',
        userContent: null,
        messages: [],
        createdAt: now,
   ...
   * @returns {Promise<void>}
   */
  async appendToAssistantMessage(groupId: GroupId, content: string, pairId: PairId | null = null): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.activePairs || !(history.activePairs instanceof Map)) {
        history.activePairs = new Map();
   ...
   * @returns {Promise<void>}
   */
  async cancelCurrentAssistantMessage(groupId: GroupId): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.activePairs || !(history.activePairs instanceof Map)) {
        history.activePairs = new Map();
      ...
      for (const [pairId, ctx] of history.activePairs.entries()) {
        ...
        history.activePairs.delete(String(pairId));
      }
   ...
   */
  async finishConversationPair(groupId: GroupId, pairId: PairId, userContent: string | null = null): Promise<unknown> {
    return this._executeForGroup(groupId, async () => {
      const history = await this._getOrInitHistory(groupId);
      if (!history.activePairs || !(history.activePairs instanceof Map)) {
        history.activePairs = new Map();
   ...
   */
  async cancelConversationPairById(groupId: GroupId, pairId: PairId): Promise<void> {
    return this._executeForGroup(groupId, async () => {
      ...
      history.conversations = history.conversations.filter((msg) => {
        const pid = msg.pairId != null ? String(msg.pairId) : '';
        return pid !== pairKey;
      });
   ...
   */
  getConversationHistory(groupId: GroupId): Array<{ role: PairRole; content: string }> {
    const history = this.histories.get(groupId);
    if (!history) {
      return [];
    }
  ...
  getConversationHistoryForContext(groupId: GroupId, options: ConversationHistoryOptions = {}): Array<{ role: PairRole; content: string }> {
    ...
    for (let idx = 0; idx < raw.length; idx++) {
      const msg = raw[idx];
      if (!msg) continue;
      const pid = msg.pairId != null ? String(msg.pairId) : `__noid_${idx}`;
    ...
    const sortPairs = (arr: ConversationPair[]) => arr.sort((a, b) => {
      const ta = typeof a.timestamp === 'number' ? a.timestamp : 0;
      const tb = typeof b.timestamp === 'number' ? b.timestamp : 0;
    ...
    let targetPairs: ConversationPair[] = [];
    if (typeof timeStart === 'number' && !Number.isNaN(timeStart)) {
      ...
      if (windowPairs.length > 0) {
        const first = windowPairs[0];
        const last = windowPairs[windowPairs.length - 1];
        const hitMinTs = first && typeof first.timestamp === 'number' ? first.timestamp : 0;
        const hitMaxTs = last && typeof last.timestamp === 'number' ? last.timestamp : 0;
    ...
    try {
      ...
      if (Number.isFinite(tokenLimit) && tokenLimit > 0 && targetPairs.length > 0) {
        ...
        const countPairTokens = (pair: ConversationPair) => {
  ...
  getLastAssistantMessageContent(groupId: GroupId): string | null {
    const history = this.histories.get(groupId);
    if (
      !history ||
   ...
   */
  getPendingMessageCount(groupId: GroupId): number {
    const history = this.histories.get(groupId);
   ...
   */
  getConversationPairCount(groupId: GroupId): number {
    const history = this.histories.get(groupId);
   ...
   */
  getConversationPairSlice(groupId: GroupId, start: number, end: number): { conversations: Array<{ role: PairRole; content: string }>; timeStart: number | null; timeEnd: number | null } {
    const history = this.histories.get(groupId);
    if (!history || !Array.isArray(history.conversations) || history.conversations.length === 0) {
    ...
    const conversationsForContext: Array<{ role: PairRole; content: string }> = [];
    for (const p of slicedPairs) {
      const msgs = Array.isArray(p.messages) ? p.messages : [];
      for (const m of msgs) {
        if (!m || typeof m !== 'object') continue;
   ...
   * @returns {Promise<void>}
   */
  async clearGroup(groupId: GroupId): Promise<void> {
    return this._executeForGroup(groupId, async () => {
   ...
   * @returns {Object} 统计信息
   */
  getStats(): { totalGroups: number; groups: Record<string, { conversationPairs: number; pendingMessages: number; isReplying: boolean }> } {
    const stats: { totalGroups: number; groups: Record<string, { conversationPairs: number; pendingMessages: number; isReplying: boolean }> } = {
