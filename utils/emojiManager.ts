...
type EmojiConfig = Record<string, string>;

type EmojiLoadResult = {
  map: EmojiConfig;
  items: EmojiItem[];
};
 ...
 * 表情包配置缓存
 */
let emojiConfig: EmojiConfig | null = null;
let emojiItems: EmojiItem[] | null = null;
let lastLoadTime = 0;
...
function loadFromJson(): EmojiLoadResult {
  try {
 ...
 */
function loadEmojiConfig(): EmojiConfig {
  const now = Date.now();
 ...
 */
export function getEmojiPath(filename: string): string | null {
 ...
 */
export function getAvailableEmojis(): Array<{ filename: string; description: string; path: string; tags: string[] }> {
  const config = loadEmojiConfig();
  const emojis: Array<{ filename: string; description: string; path: string; tags: string[] }> = [];

  const byFilename = new Map<string, EmojiItem>();
 ...
 */
export function generateEmojiPrompt(): string {
  const config = loadEmojiConfig();
  ...
  const items: EmojiItem[] = Array.isArray(emojiItems) ? emojiItems : [];
  const hasTags = items.some((it) => Array.isArray(it.tags) && it.tags.length);

  const toRow = (filename: string, description: string) => {
    const fullPath = getEmojiPath(filename);
    if (!fullPath) return '';
  ...
  for (const it of items) {
    ...
    for (const t of bucket) {
      ...
      if (list) list.push([filename, description]);
    }
  }
 ...
 */
export function generateEmojiMarkdown(): string {
  const config = loadEmojiConfig();
  const entries = Object.entries(config);
 ...
 * @returns {boolean}
 */
export function isValidEmoji(filename: string): boolean {
 ...
 * @returns {string}
 */
export function getEmojiDirectory(): string {
  return EMOJI_DIR;
}
 ...
 * 重新加载表情包配置（清除缓存）
 */
export function reloadEmojiConfig(): EmojiConfig {
  emojiConfig = null;
  emojiItems = null;
  lastLoadTime = 0;
